...
- **「コルーチン／グリーンスレッドなら何でも勝手に並行化されるので、重い処理を普通に呼んでよい」は誤り。** 協調的モデルでは中断点でしか切り替わりません。`async` 対応でないブロッキング処理（同期I/O・重いCPU処理）をコルーチン内で素直に呼ぶと、イベントループ全体が止まります（`asyncio` は別スレッド実行の `asyncio.to_thread()` を用意）。仮想スレッドでも、JDK が unmount 対応していない一部の操作（一部のファイル操作や `Object.wait()` など）は担体OSスレッドごとブロックし得ます（これらのピン留め挙動は Java 21 / JEP 444 時点の仕様であり、例えば `synchronized` によるピン留めは後の JDK（JEP 491, Java 24）で解消されています）。
